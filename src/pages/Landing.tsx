...
export default function Landing() {
  ...
  const handleSubscribe = (e: React.FormEvent) => {
    ...
    // Save email & redirect into the app
    sessionStorage.setItem("bloom:subscriber", email.trim());
    navigate("/app");
  };
  ...
  return (
    <div className="min-h-screen overflow-x-hidden relative">
      ...
      <div className="relative z-10">
        ...
        {/* Hero */}
        <section className="relative px-4 pt-8 pb-16 bg-dots/70">
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center relative">
              ...
              <div className="order-1 md:order-2 text-center md:text-left">
                ...
                <div className="mt-7 flex flex-wrap gap-3 justify-center md:justify-start">
                  <a href="#subscribe" className="inline-flex items-center gap-2 bg-gradient-pink text-primary-foreground font-bold px-6 py-3 rounded-full shadow-pink hover:scale-105 transition">
                    Start Blooming <ArrowRight className="size-4"/>
                  </a>
                  <Link to="/app" className="inline-flex items-center gap-2 bg-card text-primary font-bold px-6 py-3 rounded-full border-2 border-primary hover:bg-primary hover:text-primary-foreground transition">
                    Open App
                  </Link>
                </div>
              </div>
        ...
        {/* Tools cards */}
        <section id="tools" className="px-4 py-16 bg-checker/70 relative">
            ...
            {tools.map(({ icon: Icon, title, desc, img, tag }, i) => (
              <article key={title} className={`group bg-card rounded-3xl overflow-hidden shadow-soft hover:shadow-pink transition hover:-translate-y-1 border-pop ${i % 2 === 0 ? '' : 'rotate-[-1deg] hover:rotate-0'}`}>
                ...
                <div className="p-5">
                  ...
                  <a href="#subscribe" className="inline-flex items-center gap-1 mt-4 text-hot font-bold text-sm">
        ...
        {/* Subscribe → redirects to /app */}
        <section id="subscribe" className="px-4 py-12 bg-dots-lg/70">
          <div className="max-w-4xl mx-auto bg-gradient-pink rounded-[2.5rem] p-8 sm:p-12 text-center text-primary-foreground shadow-pink relative overflow-hidden border-pop">
            ...
            <h2 className="font-display text-4xl sm:text-5xl">Join Bloom</h2>
            <p className="font-script text-2xl mt-1">your journey starts with one step 🌸</p>
            <p className="mt-3 opacity-90 text-sm sm:text-base">Enter your email and get instant access to your Bloom dashboard.</p>
            <form onSubmit={handleSubscribe} className="mt-6 flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              ...
              />
              <button type="submit" className="bg-card text-primary font-bold px-6 py-3 rounded-full hover:scale-105 transition shadow-soft">
                Start Blooming 💕
              </button>
            </form>
          </div>
